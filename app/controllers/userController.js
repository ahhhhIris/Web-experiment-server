...
module.exports = {
   ...
   */
   Logout: async ctx => {
    const userId1 = ctx.session.user.user_id;
    ...
    try {
      ...
      // 在数据库中将用户状态设置为已注销
      let logoutResult1 = await userDao.Logout1(userId1);
      let logoutResult2 = await userDao.Logout2(userId2);
      if (logoutResult1.affectedRows === 1) {
        ctx.body = {
          code: '001',
          msg: '注销成功'
        };
